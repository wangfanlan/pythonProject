...
def get_ticket(driver, url):
    ...
    driver.find_element(by=By.XPATH, value='//*[@id="fromStationText"]').send_keys("杭州")
    time.sleep(1)
    ...
    while True:
        ...
        for ticket in tickets:
            ...
            # if ticket.find_element(by=By.CLASS_NAME, value='number').text == "G1544" and ticket.find_element(by=By.XPATH, value='//td[8]').text != "候补":
            # 点击预订
            # print(ticket.find_element(by=By.CLASS_NAME,value='cdz').text)
            # time.sleep(1)
            element = ticket.find_element(by=By.CLASS_NAME, value='btn72')
            if len(element) == 1:
                element.click()
            ...
            # 确认提交订单，然后这里和上面是一样的
                ...
                print(f"G1544次列车抢票成功，请尽快在10分钟内支付！")
                return
